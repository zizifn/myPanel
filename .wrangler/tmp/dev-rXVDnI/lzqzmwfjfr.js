// .wrangler/tmp/bundle-37XNGr/checked-fetch.js
...
// .wrangler/tmp/pages-yuFks5/bundledWorker-0.35878638603699087.mjs
...
async function S(t, { request: e, assetsFetcher: r, ctx: s }, { path: a, searchParams: n }) {
  ...
  try {
    switch (t?.type) {
      ...
      case "override": {
        i = _(await r.fetch(U(c, t.path ?? a))), t.headers && g(i.headers, t.headers);
        break;
      }
      case "static": {
        i = await r.fetch(U(c, a));
        break;
      }
      default:
...
var E = class {
  ...
  applyLocaleRedirects(e) {
    if (!e.locale?.redirect || (this.locales || (this.locales = {}), Object.assign(this.locales, e.locale.redirect), !/^\^(.)*$/.test(e.src) && e.src !== this.path) || this.headers.normal.has("location"))
      return;
    let { locale: { redirect: s, cookie: a } } = e, n = a && this.cookies[a], i = V(n ?? ""), o = V(this.reqCtx.request.headers.get("accept-language") ?? ""), d = [...i, ...o].map((m) => s[m]).filter(Boolean)[0];
    if (d) {
...
// .wrangler/tmp/pages-yuFks5/lzqzmwfjfr.js
...
// .wrangler/tmp/bundle-37XNGr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-37XNGr/middleware-loader.entry.ts
 ...
 * MIT Licensed
 */
//# sourceMappingURL=lzqzmwfjfr.js.map
